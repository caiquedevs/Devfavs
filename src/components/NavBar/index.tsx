...
export default function Navbar(): ReactElement {
  const handleChangeFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    function replaceHtml(html: any) {
      ...
      html = html.replaceAll("<H3 ", "</ul><ul><H3 ");
      ...
      categories.map((ul, index) => {
        ...
        let name = categories[index].getElementsByTagName("h3")[0].innerHTML;

        // Copia os itens da lista e transforma em ARRAY
        let linkElements = categories[index].getElementsByTagName("a");
        var copyLinkElements = Array.from(linkElements);
        ...
        copyLinkElements.map((site) => {
          sites.push({
            url: site.href,
            icon: site.attributes[2]?.nodeValue || "",
            name: site.innerHTML,
          });
    ...
    reader.onload = function (evt: any) {
      ...
      localStorage.setItem("devfavs", JSON.stringify(data));
    };
  ...
  return (
    <Nav>
      <label htmlFor="file">
        <input type="file" name="file" id="file" onChange={handleChangeFile} />
